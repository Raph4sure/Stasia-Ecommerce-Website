...
import { X, Copy, Check, Phone, ShieldCheck, Tag, Info, AlertTriangle, Sparkles } from 'lucide-react';
            ...
            <div>
              ...
              <div className="flex items-center justify-between gap-2 mb-3">
                ...
                {isOutOfStock ? (
                  ...
                  <span className="text-xs font-bold text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/70 px-3 py-1 rounded-full border border-amber-200 dark:border-amber-800/60 flex items-center gap-1">
                    <AlertTriangle className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" /> Limited Stock Available
                  </span>
                ) : (
              ...
              <div className="bg-stone-100 dark:bg-stone-950/80 rounded-2xl p-3.5 border border-stone-200/80 dark:border-stone-800 mb-6 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm font-mono text-stone-700 dark:text-stone-300">
                    Code No. <strong className="text-stone-950 dark:text-stone-100 font-bold">{product.codeNo}</strong>
                  </div>
                </div>
                <button
              ...
              <div className="bg-amber-50/70 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-800/60 rounded-2xl p-4 text-xs text-stone-700 dark:text-stone-300 mb-6 space-y-2">
                ...
                <p className="leading-relaxed">
                  To order or ask questions about this item, quote Code No. <strong className="font-mono text-stone-950 dark:text-stone-100 font-bold">{product.codeNo}</strong> to our store staff on WhatsApp, phone call, or when you visit our store. Staff will confirm availability and prepare your order immediately.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-4 border-t border-stone-100 dark:border-stone-800">
              <a
                href={`mailto:orders@stasiafabrique.com?subject=Inquiry%20for%20Item%20${encodeURIComponent(
                  product.codeNo
                )}%20-%20${encodeURIComponent(product.title)}&body=Hello%20Stasia%20Elegant%20Fabrique,%0A%0AI%20would%20like%20to%20order%20or%20inquire%20about%20item%20code:%20${encodeURIComponent(
                  product.codeNo
                )}%20(${encodeURIComponent(product.title)})%20priced%20at%20${encodeURIComponent(
                  formatPrice(product.pricePerUnit)
                )}.`}
              ...
              >
                ...
                <span>Contact Store to Order (Code: {product.codeNo})</span>
              </a>
              <button
                onClick={onClose}
