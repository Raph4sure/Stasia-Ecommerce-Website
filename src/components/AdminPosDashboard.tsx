...
export const AdminPosDashboard: React.FC<AdminPosDashboardProps> = ({
  ...
  const [historyTimeframe, setHistoryTimeframe] = useState<'daily' | 'weekly'>('daily');
  const [staffSales, setStaffSales] = useState<Sale[]>([]);
  const [historySummary, setHistorySummary] = useState({ totalRevenue: 0, totalUnitsSold: 0, transactionsCount: 0 });
    ...
    setCart((prev) => {
      ...
      if (existing) {
        ...
        if (existing.quantity >= product.quantityInStock) {
          alert(`Cannot add more. Current available stock limit is ${product.quantityInStock}.`);
          return prev;
        }
  ...
  const updateQuantity = (productId: number, newQty: number) => {
    ...
    if (newQty > product.quantityInStock) {
      alert(`Maximum stock available for "${product.title}" is ${product.quantityInStock}`);
      return;
    }
  ...
  const cartTotal = useMemo(() => {
    return cart.reduce((sum, item) => sum + item.product.pricePerUnit * item.quantity, 0);
  }, [cart]);
  ...
  const handleProcessSale = async () => {
    ...
    try {
      ...
      setSaleError(err.message || 'Transaction failed. Please check available stock levels.');
    } finally {
  ...
  const fetchStaffHistory = async (timeframe: 'daily' | 'weekly' = historyTimeframe) => {
    setIsLoadingHistory(true);
    try {
      const data = await fetchWithAuth(`/api/sales?timeframe=${timeframe}`);
      setStaffSales(data.sales || []);
      setHistorySummary(data.summary || { totalRevenue: 0, totalUnitsSold: 0, transactionsCount: 0 });
    } catch (err) {
  ...
  return (
    <div id="admin-pos-dashboard" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      {/* Top Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 mb-6 border-b border-stone-200">
        ...
        <div className="flex items-center gap-2">
          {currentUser.role === 'SUPER_ADMIN' && onSwitchToSuperAdmin && (
            <button
            ...
            <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm space-y-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                />
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {posProducts.length === 0 ? (
                ...
                posProducts.map((p) => {
                  ...
                  const isLowStock = p.quantityInStock > 0 && p.quantityInStock < 5;
                  const thumb =
                    p.images.length > 0
                      ? p.images[0]
                      ...
                      <div>
                        ...
                        <div className="aspect-[4/3] rounded-lg overflow-hidden bg-stone-100 mb-2 relative">
                          ...
                          <div className="absolute top-1 right-1">
                            ...
                            >
                              {isOutOfStock ? '0 Left' : `${p.quantityInStock} in Stock`}
                            </span>
                          </div>
                        </div>
              ...
              <div>
                ...
                {cart.length === 0 ? (
                  <div className="py-16 text-center text-stone-400">
                    ...
                    <p className="text-xs font-medium text-stone-600">Cart is currently empty</p>
                    <p className="text-[11px] text-stone-400 mt-1">
                      Click any item on the left to add it to the transaction register.
                    </p>
                  </div>
                ) : (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-[11px] text-stone-500 font-mono font-normal">
                            Code: {item.product.codeNo} • {formatPrice(item.product.pricePerUnit)} ea.
                          </div>
                          <div className="text-[11px] font-mono font-semibold text-stone-900 mt-0.5">
                            Subtotal: {formatPrice(item.product.pricePerUnit * item.quantity)}
                          </div>
                        </div>
              ...
              <div className="pt-4 border-t border-stone-200 mt-4 space-y-3">
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between text-stone-500">
                    ...
                    <span className="font-mono font-semibold text-stone-800">{cartUnits} Units</span>
                  </div>
                  <div className="flex justify-between text-stone-500">
                ...
                >
                  {isProcessingSale ? (
                    ...
                    <>
                      ...
                      <span>Process Sale ({formatPrice(cartTotal)})</span>
                    </>
                  )}
                </button>
              </div>
      ...
      {activeTab === 'history' && (
        <div className="space-y-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-900 flex items-start gap-2.5">
            ...
            <div>
              <span className="font-semibold">Permission Restricted View:</span> Sales staff access is restricted to
              immediate sales operations and time-bound transaction logs ({historyTimeframe === 'daily' ? "Today's" : "This Week's"} records only).
            </div>
          </div>
          ...
          <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
            <div className="flex items-center gap-2">
              <span className="text-xs text-stone-500">Permitted Time Window:</span>
              <button
                onClick={() => {
            ...
            <div className="text-xs font-mono font-semibold text-stone-900">
              Total Recorded: <span className="text-emerald-700">{formatPrice(historySummary.totalRevenue)}</span> ({historySummary.totalUnitsSold} units)
            </div>
          </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-stone-100 text-stone-700 uppercase font-semibold border-b border-stone-200">
                  <tr>
                    ...
                    <th className="py-2.5 px-4">Code No.</th>
                  ...
                  {isLoadingHistory ? (
                    <tr>
                      <td colSpan={7} className="py-8 text-center text-stone-400">
                        Loading authorized sales records...
                      </td>
                    </tr>
                  ) : staffSales.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-8 text-center text-stone-400">
                        No transactions found for this window.
                      </td>
                    </tr>
                  ) : (
                    staffSales.map((s) => (
                      <tr key={s.id} className="hover:bg-stone-50">
                        ...
                        <td className="py-2.5 px-4 font-medium text-stone-900">{s.productTitle}</td>
          ...
          <div className="bg-white dark:bg-stone-900 rounded-3xl max-w-md w-full p-6 shadow-2xl border border-stone-200 dark:border-stone-800 text-stone-900 dark:text-stone-100">
            <div className="text-center pb-4 border-b border-stone-200 dark:border-stone-800">
              ...
              <p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5 font-mono">
                Stock automatically decremented in showroom inventory
              </p>
            </div>
            ...
            <div className="py-4 space-y-3 font-mono text-xs border-b border-stone-200 dark:border-stone-800">
              <div className="flex justify-between text-stone-500 dark:text-stone-400">
                <span>Date & Time</span>
                <span>{formatDateTime(completedSaleData.timestamp)}</span>
              </div>
              <div className="flex justify-between text-stone-500 dark:text-stone-400">
              ...
              <div className="pt-2 border-t border-dashed border-stone-200 dark:border-stone-700 space-y-2">
                {completedSaleData.sales.map((item, idx) => (
                  <div key={idx} className="flex justify-between">
                    <div>
                      <div className="font-sans font-medium text-stone-800 dark:text-stone-200">{item.productTitle}</div>
                      <div className="text-[10px] text-stone-400 dark:text-stone-500">
                        {item.productCode} × {item.quantitySold}
                      </div>
                    </div>
              ...
              <div className="pt-2 border-t border-stone-200 dark:border-stone-800 flex justify-between text-sm font-bold text-stone-950 dark:text-stone-50">
                ...
                <span className="text-emerald-700 dark:text-emerald-400">{formatPrice(completedSaleData.total)}</span>
              </div>
            </div>
