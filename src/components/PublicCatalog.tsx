...
export const PublicCatalog: React.FC<PublicCatalogProps> = ({
  ...
  const [isNoticePopupOpen, setIsNoticePopupOpen] = useState<boolean>(() => {
    // Check if user already dismissed it this session
    try {
      return sessionStorage.getItem('stasia_hide_notice_popup') !== 'true';
    } catch {
      return true;
    }
  });

  // Row-by-row lazy loading state (starts with 1 row = 4 items)
  const [visibleCount, setVisibleCount] = useState<number>(ITEMS_PER_ROW);
  const [isLoadingMore, setIsLoadingMore] = useState<boolean>(false);
  const sentinelRef = useRef<HTMLDivElement | null>(null);

  // Available categories for public
  const availableCategoryList = useMemo(() => {
    const activeCats = categories.filter((c) => c.isAvailable).map((c) => c.category);
  ...
  // Reset row-by-row pagination to 1 row whenever filters or search change
  useEffect(() => {
    setVisibleCount(ITEMS_PER_ROW);
  }, [selectedCategory, searchQuery, sortBy]);

  // Sliced products for row-by-row display
  ...
  // Load next row function
  const loadNextRow = useCallback(() => {
    if (visibleCount >= filteredProducts.length || isLoadingMore) return;
    setIsLoadingMore(true);
    setTimeout(() => {
      setVisibleCount((prev) => Math.min(prev + ITEMS_PER_ROW, filteredProducts.length));
  ...
  return (
    <div id="public-catalog-container" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        >
          ...
          >
            ...
            <h2 className="font-serif text-2xl sm:text-3xl font-bold tracking-tight text-stone-900 dark:text-stone-50 mb-3">
              Quality Fabrics & Wears, <span className="italic font-normal text-amber-700 dark:text-amber-400">Made for Elegance.</span>
            </h2>

            {/* Brand Notice */}
            <p className="text-stone-600 dark:text-stone-300 text-xs sm:text-sm leading-relaxed mb-6 font-sans">
              Welcome to our store. We have original fabrics, George and lace wrappers, premium Ankara silks, stylish readymade clothes, and matching leather bags.
              Every item has a registered <strong className="text-amber-800 dark:text-amber-300 font-semibold font-mono">Code No.</strong> so you can easily order or buy directly from our store.
            </p>
            ...
            <div className="rounded-2xl p-4 sm:p-5 bg-stone-50 dark:bg-stone-950/70 border border-stone-200/80 dark:border-stone-800 mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-stone-600 dark:text-stone-400">
                <div className="p-3 rounded-xl bg-white dark:bg-stone-900/80 border border-stone-200/60 dark:border-stone-800 flex items-start gap-2.5">
                  ...
                  <div>
                    <span className="font-semibold text-stone-900 dark:text-stone-200 block mb-0.5">Copy Code No.</span>
                    Find the item you like and copy or write down the code (e.g. <code className="font-mono text-amber-700 dark:text-amber-400 font-semibold">CLT-101</code>).
                  </div>
                </div>
                ...
                <div className="p-3 rounded-xl bg-white dark:bg-stone-900/80 border border-stone-200/60 dark:border-stone-800 flex items-start gap-2.5">
                  ...
                  <div>
                    <span className="font-semibold text-stone-900 dark:text-stone-200 block mb-0.5">Message or Call Us</span>
                    Tell our store staff the Code No. to confirm availability, color, or your exact size.
                  </div>
                </div>
                ...
                <div className="p-3 rounded-xl bg-white dark:bg-stone-900/80 border border-stone-200/60 dark:border-stone-800 flex items-start gap-2.5">
                  ...
                  <div>
                    <span className="font-semibold text-stone-900 dark:text-stone-200 block mb-0.5">Direct Purchase</span>
                    Staff will quickly prepare your item, complete the sale, and provide your receipt.
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
              <span className="text-[11px] text-stone-500 dark:text-stone-400 text-center sm:text-left">
                You can reopen this guide anytime by clicking <strong className="font-semibold text-amber-700 dark:text-amber-400">&quot;How to Order&quot;</strong> at the top.
              </span>
              <button
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 mb-4 border-b border-stone-200/70 dark:border-stone-800">
        <div>
          ...
          <p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5">
            Luxury Fabrics, Ready-to-Wear Fashion & High-Quality Accessories
          </p>
        </div>

        {/* Reopen How-to-Order Pop-up Trigger */}
        <button
          id="btn-reopen-ordering-guide"
          type="button"
          onClick={() => setIsNoticePopupOpen(true)}
          className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-300 text-xs font-medium hover:bg-amber-100 dark:hover:bg-amber-900/60 transition shadow-2xs self-start sm:self-auto cursor-pointer"
        >
          <HelpCircle className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
          <span>How to Order with Code No.</span>
        </button>
      </div>
      ...
      <div id="catalog-controls" className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-stone-500 dark:text-stone-400 mb-5">
        <div className="flex items-center gap-2">
          <span>
            {searchQuery ? (
              <>Search results for &quot;<strong className="text-stone-900 dark:text-stone-100">{searchQuery}</strong>&quot;</>
            ) : selectedCategory !== 'All' ? (
              <>Showing <strong className="text-stone-900 dark:text-stone-100">{selectedCategory}</strong> collection</>
            ) : (
        ...
        {copiedCode && (
          <span className="inline-flex items-center gap-1.5 text-emerald-800 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/70 px-3 py-1 rounded-full border border-emerald-200 dark:border-emerald-800 font-mono text-xs shadow-xs animate-in fade-in">
            ...
            Code <strong className="underline">{copiedCode}</strong> copied to clipboard!
          </span>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        >
          ...
          <h3 className="font-serif text-xl font-bold text-stone-900 dark:text-stone-100 mb-2">No items found</h3>
          <p className="text-xs sm:text-sm text-stone-600 dark:text-stone-400 mb-6 leading-relaxed">
            No pieces match &quot;{searchQuery || selectedCategory}&quot;. Try selecting another category or clearing your search.
          </p>
          <button
            onClick={() => {
                  ...
                  <div className="relative w-full aspect-[4/5] bg-stone-100 dark:bg-stone-800 overflow-hidden">
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-stone-950/90 via-stone-950/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-between text-white text-xs">
                      <span className="flex items-center gap-1.5 font-medium">
                        <Eye className="w-4 h-4 text-amber-300" /> View Details
                      </span>
                  ...
                  <div className="p-5 flex flex-col flex-1 justify-between">
                    <div>
                      ...
                      <div className="flex items-center justify-between py-1.5 px-2.5 rounded-xl bg-stone-50 dark:bg-stone-950/60 border border-stone-200/70 dark:border-stone-800/80 mb-4">
                        ...
                        <button
                          type="button"
                          onClick={(e) => handleCopyCode(e, product.codeNo)}
                    ...
                    <div className="pt-3 border-t border-stone-100 dark:border-stone-800 flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-base sm:text-lg font-bold text-stone-950 dark:text-stone-50 font-sans tracking-tight">
                      ...
                      <button
                        ...
                        className="px-3.5 py-1.5 rounded-xl bg-stone-100 dark:bg-stone-800 group-hover:bg-amber-500 group-hover:text-stone-950 text-stone-800 dark:text-stone-200 text-xs font-semibold transition-all flex items-center gap-1 shadow-2xs cursor-pointer"
                      >
          ...
          {!hasMore && filteredProducts.length > 0 && (
            <div className="mt-12 py-8 text-center border-t border-stone-200/60 dark:border-stone-800">
              <span className="inline-flex items-center gap-2 text-xs text-stone-500 dark:text-stone-400 bg-stone-100/80 dark:bg-stone-900 px-4 py-2 rounded-full border border-stone-200/60 dark:border-stone-800">
                ...
                <span>You have viewed all items in this collection • Stasia Elegant Fabrique</span>
              </span>
            </div>
          )}
        </>
