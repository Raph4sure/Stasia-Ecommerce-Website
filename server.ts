...
import { users, products, productImages, sales, categorySettings, savedCarts } from './src/db/schema';
...
async function startServer() {
  ...
  app.get('/api/auth/admins', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
  ...
  app.post('/api/auth/create-admin', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
  ...
  app.patch('/api/auth/admins/:id', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (role && (role === 'ADMIN' || role === 'SUPER_ADMIN')) {
        if (targetUser.email === 'raph4sure007@gmail.com' && role !== 'SUPER_ADMIN') {
  ...
  app.delete('/api/auth/admins/:id', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const targetId = Number(req.params.id);
  ...
  app.get('/api/products', async (req: Request, res: Response) => {
    try {
      ...
      const unavailableCategoryNames = new Set(unavailableCategoriesList.map((c) => c.category));
      ...
      if (isPublic) {
        filtered = filtered.filter(
          (p) => p.isAvailable && !unavailableCategoryNames.has(p.category)
        );
      }
      ...
      if (searchQuery && searchQuery.trim()) {
        ...
        filtered = filtered.filter(
          (p) => p.title.toLowerCase().includes(q) || p.codeNo.toLowerCase().includes(q)
        );
      }
  ...
  app.post('/api/products', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { title, codeNo, category, pricePerUnit, quantityInStock, weightPerUnit, isAvailable, images } = req.body;

      if (!title || !codeNo || !category || pricePerUnit === undefined) {
        return res.status(400).json({ error: 'Title, Code No, Category, and Price per unit are required' });
      }

      const cleanCodeNo = String(codeNo).trim().toUpperCase();

      // Check unique code_no
      const existingCode = await db.select().from(products).where(eq(products.codeNo, cleanCodeNo)).limit(1);
      if (existingCode.length > 0) {
        return res.status(400).json({ error: `Product with Code No "${cleanCodeNo}" already exists.` });
      }

      const parsedWeight = Math.max(0, parseFloat(String(weightPerUnit !== undefined ? weightPerUnit : '0')) || 0);
      ...
      const newProduct = await db
        ...
        .values({
          ...
          pricePerUnit: Math.max(0, Math.round(Number(pricePerUnit))),
          quantityInStock: Math.max(0, Math.round(Number(quantityInStock || 0))),
  ...
  app.patch('/api/products/:id', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (codeNo !== undefined) {
        ...
        const existingCode = await db.select().from(products).where(eq(products.codeNo, cleanCode)).limit(1);
        if (existingCode.length > 0 && existingCode[0].id !== productId) {
          return res.status(400).json({ error: `Code No "${cleanCode}" is already in use by another product.` });
        }
        updates.codeNo = cleanCode;
      }
      if (category !== undefined) updates.category = String(category).trim();
      if (pricePerUnit !== undefined) updates.pricePerUnit = Math.max(0, Math.round(Number(pricePerUnit)));
      if (quantityInStock !== undefined) updates.quantityInStock = Math.max(0, Math.round(Number(quantityInStock)));
      if (weightPerUnit !== undefined) updates.weightPerUnit = Math.max(0, parseFloat(String(weightPerUnit)) || 0);
      ...
      const currentImages = await db.select().from(productImages).where(eq(productImages.productId, productId));
  ...
  app.delete('/api/products/:id', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const productId = Number(req.params.id);
      await db.delete(productImages).where(eq(productImages.productId, productId));
  ...
  app.get('/api/categories', async (req: Request, res: Response) => {
    try {
      const allCategories = await db.select({ category: products.category }).from(products);
      const uniqueCats = Array.from(new Set(allCategories.map((c) => c.category)));

      const settings = await db.select().from(categorySettings);
      const settingsMap = new Map(settings.map((s) => [s.category, s.isAvailable]));

      const result = uniqueCats.map((cat) => ({
        category: cat,
        isAvailable: settingsMap.has(cat) ? settingsMap.get(cat)! : true,
      }));
  ...
  app.patch('/api/categories/:name/availability', requireSuperAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      res.json({ success: true, category: categoryName, isAvailable: Boolean(isAvailable) });
    } catch (err: any) {
  ...
  app.post('/api/sales/process', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const user = req.user!;
      ...
      const fetchedProducts = await db
        .select()
        .from(products)
        .where(sql`${products.id} IN (${sql.join(productIds, sql`, `)})`);
      ...
      for (const item of cartItems) {
        ...
        if (item.quantity <= 0) {
          return res.status(400).json({ error: `Invalid quantity for ${prod.title}` });
        }
      ...
      await db.transaction(async (tx) => {
        ...
        if (orderCode && typeof orderCode === 'string') {
          ...
          await tx
            ...
            .where(eq(savedCarts.code, cleanCode));
        }
      });
      ...
      res.status(500).json({ error: err.message || 'Transaction failed while processing sale' });
    }
  });
  ...
  async function generateUniqueOrderCode(): Promise<string> {
    for (let attempts = 0; attempts < 15; attempts++) {
      ...
      const code = `ST-${num}`;
      const existing = await db.select().from(savedCarts).where(eq(savedCarts.code, code)).limit(1);
      if (existing.length === 0) {
        return code;
      }
    }
  ...
  app.post('/api/saved-carts', async (req: Request, res: Response) => {
    try {
      ...
      const fetchedImages = await db
        .select()
        .from(productImages)
        .where(sql`${productImages.productId} IN (${sql.join(productIds, sql`, `)})`);
      ...
      for (const item of items) {
        const prod = productMap.get(Number(item.productId));
        if (!prod) {
          return res.status(400).json({ error: `Item with ID ${item.productId} not found in store` });
        }
      ...
      res.status(500).json({ error: err.message || 'Failed to save cart and generate code' });
    }
  });

  // 2. Fetch Saved Cart by Code (for seller to load order or customer to view)
  app.get('/api/saved-carts/:code', async (req: Request, res: Response) => {
    try {
      const code = req.params.code.trim().toUpperCase();
      const found = await db.select().from(savedCarts).where(eq(savedCarts.code, code)).limit(1);

      if (found.length === 0) {
        return res.status(404).json({ error: `Order code "${code}" not found. Please check and try again.` });
      }
      ...
      const productIds = parsedItems.map((i: any) => Number(i.productId));
      ...
      const itemsWithLiveData = parsedItems.map((item) => {
        ...
        return {
          ...
          currentPrice: liveProd ? liveProd.pricePerUnit : item.pricePerUnit,
        };
      });
      ...
      res.status(500).json({ error: 'Failed to retrieve saved cart' });
    }
  });

  // 3. List recent Pending Saved Carts (for Sellers in POS/Admin)
  app.get('/api/saved-carts', async (req: Request, res: Response) => {
    try {
  ...
  app.patch('/api/saved-carts/:code/status', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const code = req.params.code.trim().toUpperCase();
      ...
      await db
        ...
        .set({
          ...
          processedByUserId: status === 'PROCESSED' ? user.id : null,
        })
  ...
  app.get('/api/sales', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (user.role === 'ADMIN') {
        if (effectiveTimeframe !== 'daily' && effectiveTimeframe !== 'weekly') {
      ...
      if (effectiveTimeframe === 'daily') {
        ...
        // Past 365 days
        startBoundary = new Date(now.getTime() - 365 * 24 * 60 * 60 * 1000);
      } else if (effectiveTimeframe === 'custom' && startDateQuery && user.role === 'SUPER_ADMIN') {
      ...
      allSales.sort((a, b) => {
        if (sortBy === 'quantity') {
          return sortOrder === 'asc' ? a.quantitySold - b.quantitySold : b.quantitySold - a.quantitySold;
